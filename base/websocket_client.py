...
class BinanceWebSocketClient:  # pragma: no cover
    """
    This class is an attempt to fine-tune the ws connection to Binance
    in order to prevent the disconnection issues. It does not currently
    solve them, but leaving it JIC for the future.
    """
    ...
    def __init__(
        ...
        self.stdout: OutputWrapper = (
            stdout if stdout else OutputWrapper(sys.stdout)
        )
    ...
    async def connect(self):
        async for websocket in websockets.connect(
            ...
            try:
                ...
                logger.error(
                    f"Websocket connection to {self.url} lost! Retrying..."
                )
                continue
